---
description: "스프린트 구현. 게이트별 domain 라벨 따라 frontend-dev/backend-dev/mobile-dev/data-engineer/devops/ai-engineer dispatch → 끝나면 gate-verifier가 검증 + progress.md + 카드. 순차 기본, --parallel 병렬."
...
planning.md에 정의된 각 게이트를 **그 도메인의 전문 에이전트**에게 맡겨 구현하고, 끝나면 **gate-verifier**가 검증·기록·카드 이동까지 처리. 사용자는 평소엔 개입 X. 막힐 때만 보고받음.
...
- progress.md 없음 → 자동으로 `sprint-progress.py <project> <version>`를 호출해 스캐폴드 생성.

### Step 2 · 게이트 인벤토리

planning.md의 `## Core Gates` 섹션을 파싱해 각 게이트의 5개 필드 추출:
...
각 게이트가 progress.md에서 다음 상태 중 어느 것인지 판단:
...
```
You are <domain>-dev. Implement ONE gate of a sprint.
...
Gate to implement: <gate_id> — <heading>
  domain: <domain>
  happy:
    description: <text>
...
Your task:
1. Implement the feature so all three scenarios behave as defined.
...
5. Do NOT edit ~/wiki/. Stay inside <working_directory>.
...
```
✓ G1 (backend, passed) — backend-dev 3 commits, verifier all green, 카드 Done
⚠️ G2 (frontend, partial) — happy 통과, isolation 명령 실패 (자세히 보려면 progress.md)
✗ G3 (mobile, incomplete) — mobile-dev 보고: simulator 부팅 실패
```
...
1. NEVER: planning.md 게이트 정의가 incomplete인 채로 도메인 에이전트 dispatch.
...
1. ALWAYS: 게이트 dispatch 전에 그 게이트가 progress.md에서 done인지 확인 (idempotent — 재실행해도 done은 skip).
2. ALWAYS: 도메인 에이전트 prompt에 게이트의 5개 필드 모두 풀어 전달 (요약 X).
