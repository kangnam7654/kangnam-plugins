...
import type { CardFilters } from "../shared/types.js";
...
app.get("/api/cards", asyncHandler(async (req) => {
  ...
  const filters: CardFilters = {
    ...
    assigneeKind: asString(req.query.assigneeKind) as CardFilters["assigneeKind"],
...
function asyncHandler<T>(handler: (req: express.Request, res: express.Response) => Promise<T> | T): express.RequestHandler {
  return (req, res, next) => {
...
function storeForReq(req: express.Request): KanbanStore {
  const body = typeof req.body === "object" && req.body !== null ? (req.body as { cwd?: unknown }) : {};
