...
export class KanbanStore {
  ...
  async updateCard(input: UpdateCardInput): Promise<KanbanCard> {
    ...
    return this.mutate((data) => {
      ...
      if (input.description !== undefined) card.description = input.description.trim();
      if (input.kind !== undefined) {
        validateCardKind(input.kind);
        card.kind = input.kind;
        if (input.kind === "epic") delete card.epicId;
      }
      if (input.epicId !== undefined) {
        ...
        if (!epicId) {
          ...
          if (!epic) throw new KanbanStoreError(`Epic ${epicId} was not found.`, 404);
          if (epic.kind !== "epic") throw new KanbanStoreError(`${epicId} is not an epic card.`);
          if (card.kind === "epic") throw new KanbanStoreError("Epic cards cannot be nested under another epic.");
          card.epicId = epicId;
        }
      }
      if (input.sprint !== undefined) {
        ...
        else delete card.sprint;
      }
      if (input.gate !== undefined) {
        const gate = input.gate?.trim() || "";
        if (gate) card.gate = gate;
        else delete card.gate;
      }
      if (input.status !== undefined) {
        validateStatus(input.status);
        card.status = input.status;
        if (input.status === "done") card.completedAt = card.completedAt ?? new Date().toISOString();
      ...
      if (input.branch !== undefined) {
        ...
        else delete card.branch;
      }
      if (input.tags !== undefined) card.tags = input.tags === null ? [] : sanitizeTags(input.tags);
...
function applyFilters(cards: KanbanCard[], filters: CardFilters): KanbanCard[] {
  ...
  return cards.filter((card) => {
    ...
    if (filters.gate && card.gate !== filters.gate) return false;
