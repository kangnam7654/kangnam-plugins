...
const server = new McpServer({
    ...
    instructions: "Project-local Kanban adapter. Prefer the CLI directly when available. Through MCP, call kanban_context at session start and kanban_run for explicit agent-kanban CLI commands."
});
...
server.registerTool("kanban_run", {
    title: "Run Agent Kanban CLI",
    description: "Run one agent-kanban CLI command. Examples: ['create','Fix login','--cwd','/repo','--status','ready'], ['claim','AK-1','--cwd','/repo','--session','s1'], ['progress','AK-1','--cwd','/repo','--msg','Implemented parser','--files','src/parser.ts'].",
    inputSchema: z
        .object({
...
async function runCliTool(args, cwd) {
    try {
...
async function runAgentKanban(args, cwd) {
